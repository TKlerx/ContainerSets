...
$zip   = [System.IO.Compression.ZipArchive]::new($zipFs, [System.IO.Compression.ZipArchiveMode]::Create, $false)

try {
  $stageFull = (Resolve-Path $stageDir).Path.TrimEnd([System.IO.Path]::DirectorySeparatorChar, [System.IO.Path]::AltDirectorySeparatorChar)

  Get-ChildItem -Path $stageFull -Recurse -File | ForEach-Object {
    $fileFull = $_.FullName

    # relativer Pfad
    $rel = $fileFull.Substring($stageFull.Length).TrimStart([System.IO.Path]::DirectorySeparatorChar, [System.IO.Path]::AltDirectorySeparatorChar)
